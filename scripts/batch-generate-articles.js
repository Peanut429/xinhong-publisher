...
const phoneNumberMap = {
  ...
  // "02071f5f-16ba-4d98-8c19-79ccdfc6208d": "18930997147", // 铃音
};
...
const accountNames = {
  ...
  "02071f5f-16ba-4d98-8c19-79ccdfc6208d": "铃音",
};
...
const CONFIG = {
  ...
  API_URL: `http://localhost:${port}/api/write-note-enhanced`, // API地址
  LOG_FILE: "batch-generate-log.json", // 日志文件
};
 ...
 * 调用API生成文章
 */
async function generateArticle(accountId, phoneNumber) {
  ...
  for (let i = 0; i < CONFIG.ARTICLES_PER_ACCOUNT; i++) {
    ...
    try {
      ...
      const data = await retry(async () => {
        return await generateArticle(accountId, phoneNumber);
      });
